...
export async function assembleContext({
  ...
  if (includeGraph) {
    // TODO: Phase B
    // 1. Extract entity names from top chunks (source_entity values)
    // 2. Query graph_edges WHERE source_entity IN (...) AND confidence >= 0.7
    // 3. Fetch thought rows for neighbor source_thought_ids (deduplicate vs chunks)
    // 4. Append to graphNeighbors
    console.warn('[context-assembler] --graph requested but Phase B (graph_edges) is not yet implemented.');
  }
  ...
  if (graphNeighbors.length > 0) {
    sections.push(
      '[Graph Expansion]\n\n' +
      graphNeighbors.map(formatChunk).join('\n\n---\n\n')
    );
  }
